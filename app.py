...
def build_chart(df, ticker, is_weekly, is_mf, show_rv, show_roc):
    ...
    if has_vol:
        ...
        if v_max > 0:
            vol_norm = (display["Volume"] / v_max) * (p_range * 0.12) + p_lo
    ...
    # ── Z-Score annotation — top-right, above price bars ─────────────────
    z = calc_zscore(df)
    if z is not None and abs(z) >= 2.0:
        lbl     = "OVERBOUGHT" if z > 0 else "OVERSOLD"
        col_ann = C["ob"] if z > 0 else C["os_"]
        fig.add_annotation(
            text=f"<b>{lbl}</b>  Z={z:+.2f}",
            x=0.995, y=0.995, xref="paper", yref="paper",
            xanchor="right", yanchor="top", showarrow=False,
            font=dict(color=col_ann, size=11, family="IBM Plex Mono"),
    ...
    fig.update_yaxes(
        range=[-mom_abs * 6, mom_abs * 6],   # compress bars to look subtle
...
app.layout = html.Div(id="root", style={"background": C["bg"], "minHeight": "100vh"}, children=[
    ...
    html.Div([
        ...
        html.Button("RV", id="toggle-rv",  n_clicks=1,
        ...
        html.Button("RoC", id="toggle-roc", n_clicks=1,
            style={"fontFamily": "IBM Plex Mono", "fontSize": "11px", "fontWeight": "600",
                   "padding": "3px 10px", "borderRadius": "4px", "cursor": "pointer",
...
def update_chart(n_load, tf, n_rv, n_roc, ticker, asset_type):
    ...
    z_str  = f"  Z={s['z']:+.2f}" if s['z'] is not None else ""
    status = (f"{ticker}  ·  {'Mut.Fund' if is_mf else 'ETF/Stock'}  ·  "
              f"${s['last']}  |  Trade: {s['trade']}  |  Trend: {s['trend']}{z_str}")
